...
import com.codecademy.goldmedal.respository.CountryRepository;
import com.codecademy.goldmedal.respository.GoldMedalRepository;
...
public class GoldMedalController {
    ...
    private CountryMedalsListResponse getCountryMedalsListResponse(String countryName, String sortBy, boolean ascendingOrder) {
        List<GoldMedal> medalsList = goldMedalRepository.findAllByCountry(countryName, sortBy(sortBy, ascendingOrder));
    ...
    private CountryDetailsResponse getCountryDetailsResponse(String countryName) {
        ...
        var summerWins = goldMedalRepository.findAllByCountryAndSeasonOrderByYearAsc(countryName, "Summer");// TODO: get the collection of wins at the Summer Olympics, sorted by year in ascending order
        var numberSummerWins = summerWins.size() > 0 ? summerWins.size() : null;
        var totalSummerEvents = goldMedalRepository.countDistinctEventsBySeason("Summer");// TODO: get the total number of events at the Summer Olympics
        var percentageTotalSummerWins = totalSummerEvents != 0 && numberSummerWins != null ? (float) summerWins.size() / totalSummerEvents : null;
        var yearFirstSummerWin = summerWins.size() > 0 ? summerWins.get(0).getYear() : null;

        var winterWins = goldMedalRepository.findAllByCountryAndSeasonOrderByYearAsc(countryName, "Winter");// TODO: get the collection of wins at the Winter Olympics
        var numberWinterWins = winterWins.size() > 0 ? winterWins.size() : null;
        var totalWinterEvents = goldMedalRepository.countDistinctEventsBySeason("Winter");// TODO: get the total number of events at the Winter Olympics, sorted by year in ascending order
    ...
    private List<CountrySummary> getCountrySummaries(String sortBy, boolean ascendingOrder) {
        ...
        switch (sortBy) {
            case "name":
                ...
                countries = countryRepository.findAll(sortBy("name", ascendingOrder));
                break;
            case "gdp":
                countries = countryRepository.findAll(sortBy("gdp", ascendingOrder));// TODO: list of countries sorted by gdp in the given order
                break;
            // TODO: list of countries sorted by population in the given order
            case "medals":
            default:
                countries = countryRepository.findAll(sortBy("population", ascendingOrder));// TODO: list of countries in any order you choose; for sorting by medal count, additional logic below will handle that
                break;
        }
    ...
    private Sort sortBy(String field, boolean ascending){
        if(ascending){
